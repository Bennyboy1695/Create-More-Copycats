...
public class CarpetModel extends CopycatModel {

    protected static final AABB CUBE = new AABB(BlockPos.ZERO);

    public CarpetModel(BakedModel originalModel) {
        super(originalModel);
    }
    ...
    protected List<BakedQuad> getCroppedQuads(BlockState state, Direction side, RandomSource rand, BlockState material, ModelData wrappedData, RenderType renderType) {
        ...
        if (originalModel instanceof CarpetModel impl)
        ...
        for (BakedQuad quad : templateQuads) {
            quads.add(BakedQuadHelper.cloneWithCustomGeometry(quad, BakedModelHelper.cropAndMove(quad.getVertices(), quad.getSprite(), AABB.ofSize(Vec3.ZERO, 2, 0.125, 2), Vec3.ZERO)));
        }
        return quads;
    }
}
